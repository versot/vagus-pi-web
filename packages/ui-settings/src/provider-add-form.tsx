...
export function ProviderAddForm({ form, setForm, onAdd, canAdd, inputStyle, t }: {
  ...
  return (
    <>
      <div style={{ fontSize: 16, fontWeight: 600, color: t.color.fg }}>{tr("添加模型供应商")}</div>
      <div style={{ fontSize: 12, color: t.color.muted, margin: "4px 0 20px" }}>{tr("配置一个完全自定义的 API 端点和初始模型")}</div>
      <Field label={tr("名称")}>
        <input style={inputStyle} placeholder={tr("如：openai")} value={form.id} onChange={(e) => setForm({ ...form, id: e.target.value })} />
      </Field>
      ...
      <Field label="API Key">
        <input style={inputStyle} type="password" placeholder={tr("输入 API Key（可选）")} value={form.apiKey} onChange={(e) => setForm({ ...form, apiKey: e.target.value })} />
      </Field>
